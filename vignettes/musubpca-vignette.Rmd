...
cat("Number of blocks:", length(X_multi), "\n")
cat("Block sizes:", sapply(seq_along(X_multi), function(i) length(block_indices(X_multi, i))), "\n")
```
...
```{r column-stacked}
# Create column-stacked blocks (30 observations x 50 variables per block)
block1_T <- t(matrix(rnorm(50 * 30), 50, 30))
...
# Define clusters for the 50 variables
clus_T <- rep(1:5, each = 10)
...
X_multi_scaled[[2]] <- X_multi_scaled[[2]] * 10    # Scale up block 2
X_multi_scaled[[3]] <- X_multi_scaled[[3]] * 0.1   # Scale down block 3
