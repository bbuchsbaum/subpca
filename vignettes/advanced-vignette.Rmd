...
for(i in 1:n_clusters) {
  ...
  } else {  # keep near-sparse but avoid zero-variance blocks
    X_sparse_base[, var_idx] <- rnorm(n_obs * 10, sd = 1e-8)
  }
}
...
robust_pca <- function(X, ncomp, preproc, ind = NULL) {
  ...
  multivarious::pca(X_robust, ncomp = ncomp, preproc = fresh(pass()))
}

# Sparse PCA (simplified version)
sparse_pca <- function(X, ncomp, preproc, ind = NULL) {
  # Standard PCA first
  pca_result <- multivarious::pca(X, ncomp = ncomp, preproc = fresh(pass()))
  pp <- pca_result[['preproc']]
